let api=[
  ...
  let componentItem=document.querySelectorAll(".component-item")
  ...
  for(let x of selected){
    ...
    x.addEventListener("change",()=>{
      ...
      if(selectedValue=="Filter by Region"&&search.value==""){
        cartona=api;
      }else if(selectedValue=="Filter by Region"&&search.value!=""){
        cartona=api.filter((result)=>{
        return (result.name.includes(search.value))
        })
      }
      else{
        cartona=api.filter((result)=>{
          return (result.region.includes(selectedValue) &&result.name.includes(search.value))
        })
      }
...
function view(){
    ...
    for(let i=0;i<cartona.length;i++){
        component.innerHTML+=`
        <div onclick="viewDetails(${i})" class="component-item col-md-2 m-4 component-item">
        <img class="w-100 h-50" src="${cartona[i].flags.png}" alt="sasa">
            <h3>${cartona[i].name}</h3>
            <p>Population : ${cartona[i].population} </p>
            <p>Region :${cartona[i].region}  </p>
            <p>Capital : ${cartona[i].capital} </p>
        </div>`;
...
function viewDetails(i){
  ...
  <div class="row">
      <div class="col-md-6">
          <img class="w-100" src="${cartona[i].flags.png}" alt="">
      </div>
      <div class="col-md-6 row">
      <h1 class="col-md-12" >${cartona[i].name}</h1>
       <div class="col-md-6">
          <p><span>Native Name : </span> ${cartona[i].nativeName}</p>
          <p><span>Population : </span>${cartona[i].population} </p>
          <p><span>Region : </span> ${cartona[i].region}</p>
          <p><span>Sub Region : </span> ${cartona[i].subregion} </p>
          <p><span>Capital : </span> ${cartona[i].capital}  </p>
       </div>
       <div class="col-md-6">
          
          <p><span>Top Level Domain : </span> ${cartona[i].topLevelDomain} </p>
          <p><span>Currencies : </span> ${cartona[i].currencies[0].name}</p>
          <p><span>Languages : </span> ${cartona[i].languages[0].name}</p>
         
      </div>
      <div class="col-md-10">
           <p><span>Border Countries : </span>
           <button class="btn ">${cartona[i]?.borders}</button>
          </p>
